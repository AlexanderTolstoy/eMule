...
void CUpDownClient::Init()
{
	...
	if (m_lastRefreshedULDisplay > MIN2MS(20)) // ZZ:DownloadManager
		m_dwLastTriedToConnect = m_lastRefreshedULDisplay - MIN2MS(20);
	else
...
//fox88 ++ ban bad mods - not for release
	if (strBuffer.Find(_T("[VERYCD]XTHAME")) >= 0
		|| strBuffer.Find(_T("[CHN]SHAOHAN")) >= 0
		|| GetClientModVer().Find(_T("[xl build61]")) >= 0)
	{
		m_bGPLEvildoer = true;
		Ban(m_pszUsername);
	}
	...
	if (IsFriend())
		...
		default:
			if (ExtProtocolAvailable())
				iImage = bRatioGt1 ? 3 : 1;
			else
				iImage = bRatioGt1 ? 2 : 0;
		}
	}
		...
		if (GetMessagesSent() == 0 && GetMessagesReceived() == 0 && GetChatCaptchaState() != CA_CAPTCHASOLVED) {
			...
			if (GetChatCaptchaState() != CA_CHALLENGESENT) {
				...
				if (m_fSupportsCaptcha != NULL) {
					...
					// client doesn't supports captchas, but we require them, tell him that
					// it's not going to work out with an answer message (will not be shown
					// and doesn't counts as sent message)
