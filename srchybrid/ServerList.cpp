	...
	try {
		...
		for (INT_PTR j = 0; j < fservercount; ++j) {
			...
			if (!nextserver->GetVersion().IsEmpty()) {
				...
				tversion.WriteTagToFile(&servermet, UTR8strOptBOM);
				++uTagCount;
			}
